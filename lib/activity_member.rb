module ActivityMember
  ...
  def activity_line(activity, p_ids, names, checked, blocked_by, i_paid, age_ok)
    optns = {}
    ...
    html = %Q[
    <form class="edit_activity" id="edit_activity_#{activity.id}" action="/aktiviteter/#{activity.id}/toggle" accept-charset="UTF-8" data-remote="true" method="post">
      <input name="utf8" type="hidden" value="&#x2713;" />
      <table>
        <tr>
          <td style='width: 200px; text-align: left;'><input value="#{activity.id}" type="hidden" name="activity[member_id]" id="activity_member_id" />
