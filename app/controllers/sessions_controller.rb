...
class SessionsController < ApplicationController
  ...
  def create
    if params[:session][:username].casecmp('sommerlejr').zero? && params[:session][:password].casecmp('kartoffelmel').zero?
      guest_log_in
      redirect_to members_path
    else
      member = Member.find_by_number(params[:session][:username])
      if member && member.password && member.password.password.casecmp(params[:session][:password]).zero?
        log_in member
